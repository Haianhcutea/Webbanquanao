...
const Order = () => {
  ...
  //
  const handleQuantityChange = async (product_id, color, size, newQuantity, stock) => {
    if (newQuantity < 1) return; // Ngăn giảm số lượng xuống dưới 1.
    if (stock && newQuantity > stock) {
      message.warning("Số lượng sản phẩm đã đạt tối đa trong kho.");
      return;
    }
    const payload = {
      product_id,
                  ...
                  <form action="#">
                    <div className="cart-table table-responsive">
                      <table className="table">
                        ...
                        {console.log(cartByUserID?.cartData, "111")
                        }
                        <tbody>
                                  ...
                                  <td className="pro-quantity">
                                    <div className="quantity-control">
                                      ...
                                      <button
                                        ...
                                        onClick={(e) => {
                                          e.preventDefault();
                                          const stock = dataProduct.find(
                                            (item) => item._id === product.product_id
                                          )?.variants.find(
                                            (variantItem) => variantItem.color === variant.color && variantItem.size === variant.size
                                          )?.stock;
                                          handleQuantityChange(product.product_id, variant.color, variant.size, variant.quantity + 1, stock);
                                        }}>
                                        +
                                      </button>
                                    </div>
